...
class ExperienceMemory():
    ...
    def add(self, fi, a, r, Nfi, done):
        if r>0:
            ...
            if len(self.positive_memory) + len(experience) >= self.memory_size:
                self.positive_memory[0:(len(experience) + len(self.positive_memory)) - self.memory_size] = []
            self.positive_memory.extend(experience)
        else:
            flattened_fi = np.reshape(fi, [28224])
            flattened_Nfi = np.reshape(Nfi, [28224])
            experience = np.reshape(np.array([flattened_fi,a,r,flattened_Nfi,done]),[1,5])
            if len(self.negative_memory) + len(experience) >= self.memory_size:
                self.negative_memory[0:(len(experience) + len(self.negative_memory)) - self.memory_size] = []
            self.negative_memory.extend(experience)

    def sample(self, size):
        a = np.reshape(np.array(random.sample(self.positive_memory, 6)), [6, 5])
        b = np.reshape(np.array(random.sample(self.negative_memory, size-6)), [size-6, 5])
